    ...
    for similarId in similarIds:
        ...
        try:
            ...
            try:
                ...
                stringPrint = str(similarityProductName) + " - " + tupple_target[0][1] + " - " +str(tupple_source[0][1])
...
def performEntityResolution(driver, entitySourceId, entityTargetId):
    ...
    if(len(tupple_source_with_ingredients) == 0 or len(tupple_target_with_ingredients) == 0):
        ...
        try:
            ...
            print("Comparing = " + tupple_source_with_ingredients[0][1] + " with " + tupple_target_with_ingredients[0][1])
            similarityScoreProduct = checkSimilarityJaro(tupple_source_with_ingredients[0][1], tupple_target_with_ingredients[0][1])
            print("Similarity product = " + str(similarityScoreProduct))
            print("Comparing = " + tupple_source_with_ingredients[0][2] + " with " + tupple_target_with_ingredients[0][2])
            similarityScoreManufacturer = checkSimilarityJaro(tupple_source_with_ingredients[0][2], tupple_target_with_ingredients[0][2])
            print("Similarity manufacturer = " + str(similarityScoreManufacturer))
            ...
            similarityTotal = (0.5 * similarityScoreProduct) + (0.2 * similarityScoreManufacturer) + (0.3 * similarityScoreIngredients)
            print("Similarity Total = " + str(similarityTotal))
            ...
            if similarityTotal >= threshold:
                ...
                stringToWrite = str(entitySourceId) + "|" + str(tupple_source_with_ingredients[0][1]) + "|" + str(
                    ...
                    tupple_target_with_ingredients[0][2]) + "|" + ingredientsTarget + "|owl:sameAs|" + str(similarityTotal)
            else:
                createRdfsSeeAlsoRelationQuery(driver, entitySourceId, entityTargetId)
                stringToWrite = str(entitySourceId) + "|" + str(tupple_source_with_ingredients[0][1]) + "|" + str(
                    tupple_source_with_ingredients[0][2]) + "|" + ingredientsSource + "|" + \
                                str(entityTargetId) + "|" + str(tupple_target_with_ingredients[0][1]) + "|" + str(
                    tupple_target_with_ingredients[0][2]) + "|" + ingredientsTarget + "|rdfs:seeAlso|" + str(similarityTotal)
...
def neo4j_most_similar(driver, model, key, filename):
    with driver.session() as session:
        find_entity_query = "MATCH (m:ns1__FoodProduct {rdfs__label: '%s'}) return id(m)" % key
        ...
        for r in result:
            ...
            for s_entity in similar_entities:
                ...
                for sm in similar_entity_names:
                    ...
                    print(sm.value() +" with id "+str(s_entity[0])+" with cosine similarity "+str(s_entity[1]))
                    productName = sm.value()
                    productId = str(s_entity[0])
                    productSimilarity = str(s_entity[1])
                    jaroSimilarityScore = checkSimilarityJaro(key, productName)
                    if jaroSimilarityScore >= 0.7:
                        isRelevant = 1
                    else:
                        isRelevant = 0
                    stringWrite = key+";"+str(r.value())+";"+productName+";"+productId+";"+productSimilarity+";"+str(jaroSimilarityScore)+";"+str(isRelevant)

                    writecsv("scenarios/"+filename.replace(".emb", ""), stringWrite)
...
def performEntityResolutionOfEntity(driver, model, entity_id):
    ...
    for similarEntityId in listEntities:
        print("Comparing on entity source id = " + str(similarEntityId) +" vs entity target id "+str(similarEntityId))
...
if __name__ == '__main__':
    ...
    for i in range(1, 7):
        ...
        filename = 'emb/halalmodel'+str(i)+'.emb'
        model = KeyedVectors.load_word2vec_format(filename, binary=False)
        neo4j_most_similar(driver, model, 'Energen rasa kacang hijau', filename)
        neo4j_most_similar(driver, model, 'Nissin Wafer Krim Rasa Strawberry', filename)
        neo4j_most_similar(driver, model, 'John Farmer Peanut Pasta 16', filename)
